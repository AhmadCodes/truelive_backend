...
class Database:
    ...
    def get_view_by_id(self, view_id: str) -> View:
        with sqlite3.connect(self.db_path) as conn:
            cursor = conn.cursor()
            cursor.execute(
                "SELECT id, screen_id, name, layout_rows, layout_columns FROM views WHERE id = ?",
                (view_id,),
            )
            row = cursor.fetchone()
            if row:
    ...
    def get_views_by_screen(self, screen_id: str) -> List[View]:
        with sqlite3.connect(self.db_path) as conn:
            cursor = conn.cursor()
            cursor.execute(
                "SELECT id, screen_id, name, layout_rows, layout_columns FROM views WHERE screen_id = ?",
                (screen_id,),
            )
    ...
    def get_pc_by_id(self, pc_id: str) -> PC:
        with sqlite3.connect(self.db_path) as conn:
            cursor = conn.cursor()
            cursor.execute(
                "SELECT id, name, ip_address, gpu_type FROM pcs WHERE id = ?", (pc_id,)
            )
            row = cursor.fetchone()
            if row:
                return PC(*row)

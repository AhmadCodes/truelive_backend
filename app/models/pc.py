...
from sqlalchemy import Column, String, BigInteger, CheckConstraint, ForeignKey, Index, Text
...
class PC(BaseModel):
    ...
    id = Column(
        String(50),
        primary_key=True,
        comment="Unique identifier for the PC"
    )
    ...
    name = Column(
        String(255),
        nullable=False,
        comment="Display name of the PC"
    )
    ...
    gpu_type = Column(
        String(100),
        nullable=True,
        comment="GPU type/model installed on the PC"
    )
    ...
    auth_token = Column(
        Text,
        nullable=True,
        comment="Authentication token for PC"
    )
    ...
    last_connected = Column(
        BigInteger,
        nullable=True,
        comment="Unix timestamp of last connection"
    )
    ...
    __table_args__ = (
        CheckConstraint(
            "role IN ('controller', 'manager')",
            name='check_pc_role'
        ),
        # Indexes
        Index('idx_pcs_role', 'role'),
        Index('idx_pcs_manager_id', 'manager_id'),
        Index('idx_pcs_last_connected', 'last_connected', postgresql_ops={'last_connected': 'DESC'}),
    ...
    screens = relationship(
        ...
        doc="Screens associated with this PC"
    )
    ...
    screen_mappings = relationship(
        ...
        back_populates="pc",
        cascade="all, delete-orphan",
        doc="Screen mappings for this PC"
    )
